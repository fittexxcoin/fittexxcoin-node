#!/usr/bin/env python3
"""Script to poll fittexxcoind for MTP to elapse and broadcast a transaction"""
...
class FittexxcoinDInfo:
    ...
    def __str__(self) -> str:
        return f"FittexxcoinD at {self.url}"
...
class FittexxcoinDHelper:

    def __init__(self, info: FittexxcoinDInfo):
        self.info = info
...
async def poller(info: FittexxcoinDInfo, txnhex: str, mtptarget: int, sleeptime: float):
    bd = FittexxcoinDHelper(info)
    ...
    while True:
        ...
        try:
            ...
            # fittexxcoind possibly temporarily down, restarted, restarting, etc. Just print a message and continue looping
            # after sleeping
            print(e)
        else:
...
def main():
    if len(sys.argv) not in (7, 8):
        sys.exit(f"Usage: {sys.argv[0]} <host> <port> <rpcuser> <rpcpass> <mtptarget> <txnhex> [sleeptime]\n"
                 """
    <host>      - The IP address or host of the fittexxcoind HTTP-RPC server (e.g. localhost)
    <port>      - The port for the fittexxcoind HTTP-RPC server (e.g. 7889)
    <rpcuser>   - The -rpcuser specified in fittexxcoin.conf for the RPC server
    <rpcpass>   - The -rpcpassword specified in fittexxcoin.conf for the RPC server
...
""")
    info = FittexxcoinDInfo(*sys.argv[1:5])
