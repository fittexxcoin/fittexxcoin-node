...
def get_chainparams(rpc_caller, block):
    ...
    if chaininfo['chain'] == 'main':
        ...
        sys.exit("ScaleNet chainparams should not be updated. See fxxN issue "
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description=(
        ...
        "Prerequisites: RPC access to a fittexxcoind node.\n\n"),
        formatter_class=argparse.RawTextHelpFormatter)
    parser.add_argument('--address', '-a', default="127.0.0.1:7889",
                        help="Node address for making RPC calls.\n"
                             "The chain (MainNet, TestNet, TestNet4, ScaleNet) will be automatically detected.\n"
                             "Default: '127.0.0.1:7889'")
    ...
    parser.add_argument('--config', '-c', default="~/.fittexxcoin/fittexxcoin.conf",
                        help="Path to fittexxcoin.conf for RPC authentication arguments (rpcuser & rpcpassword).\n"
                             "Default: ~/.fittexxcoin/fittexxcoin.conf")
    ...
    if os.path.isfile(args.config):
        ...
        raise FileNotFoundError("Missing fittexxcoin.conf")
    if user is None:
