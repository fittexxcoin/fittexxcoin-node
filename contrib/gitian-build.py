...
def setup():
    ...
    if not os.path.isdir('fittexxcoin-node'):
        subprocess.check_call(
...
def build():
    ...
    base_output_dir = 'fittexxcoin-binaries/' + args.version
    os.makedirs(base_output_dir + '/src', exist_ok=True)
    print('\nBuilding Dependencies\n')
    os.chdir('gitian-builder')
    os.makedirs('inputs', exist_ok=True)

    subprocess.check_call(['make', '-C', '../fittexxcoin-node/depends',
                           'download', 'SOURCES_PATH=' + os.getcwd() + '/cache/common'])

    output_dir_src = '../' + base_output_dir + '/src'
    if args.linux:
        print('\nCompiling ' + args.version + ' Linux')
        subprocess.check_call(['bin/gbuild', '-j', args.jobs, '-m', args.memory, '--commit', 'fittexxcoin=' + args.commit,
                               '--url', 'fittexxcoin=' + args.url, '../fittexxcoin-node/contrib/gitian-descriptors/gitian-linux.yml'])
        subprocess.check_call(['bin/gsign', '-p', args.sign_prog, '--signer', args.signer, '--release', args.version +
                               '-linux', '--destination', '../gitian.sigs/', '../fittexxcoin-node/contrib/gitian-descriptors/gitian-linux.yml'])
        output_dir_linux = '../' + base_output_dir + '/linux'
        os.makedirs(output_dir_linux, exist_ok=True)
        subprocess.check_call(
            'mv build/out/fittexxcoin-*.tar.gz ' + output_dir_linux, shell=True)
        subprocess.check_call(
            'mv build/out/src/fittexxcoin-*.tar.gz ' + output_dir_src, shell=True)
        subprocess.check_call(
            'mv result/fittexxcoin-*-linux-res.yml ' + output_dir_linux, shell=True)

    if args.windows:
        print('\nCompiling ' + args.version + ' Windows')
        subprocess.check_call(['bin/gbuild', '-j', args.jobs, '-m', args.memory, '--commit', 'fittexxcoin=' + args.commit,
                               '--url', 'fittexxcoin=' + args.url, '../fittexxcoin-node/contrib/gitian-descriptors/gitian-win.yml'])
        subprocess.check_call(['bin/gsign', '-p', args.sign_prog, '--signer', args.signer, '--release', args.version +
                               '-win-unsigned', '--destination', '../gitian.sigs/', '../fittexxcoin-node/contrib/gitian-descriptors/gitian-win.yml'])
        output_dir_win = '../' + base_output_dir + '/win'
        os.makedirs(output_dir_win, exist_ok=True)
        subprocess.check_call(
            'mv build/out/fittexxcoin-*-win-unsigned.tar.gz inputs/', shell=True)
        subprocess.check_call(
            'mv build/out/fittexxcoin-*.zip build/out/fittexxcoin-*.exe ' + output_dir_win, shell=True)
        subprocess.check_call(
            'mv build/out/src/fittexxcoin-*.tar.gz ' + output_dir_src, shell=True)
        subprocess.check_call(
            'mv result/fittexxcoin-*-win-res.yml ' + output_dir_win, shell=True)

    if args.macos:
        print('\nCompiling ' + args.version + ' MacOS')
        subprocess.check_call(['bin/gbuild', '-j', args.jobs, '-m', args.memory, '--commit', 'fittexxcoin=' + args.commit,
                               '--url', 'fittexxcoin=' + args.url, '../fittexxcoin-node/contrib/gitian-descriptors/gitian-osx.yml'])
        subprocess.check_call(['bin/gsign', '-p', args.sign_prog, '--signer', args.signer, '--release', args.version +
                               '-osx-unsigned', '--destination', '../gitian.sigs/', '../fittexxcoin-node/contrib/gitian-descriptors/gitian-osx.yml'])
        output_dir_osx = '../' + base_output_dir + '/osx'
        os.makedirs(output_dir_osx, exist_ok=True)
        subprocess.check_call(
            'mv build/out/fittexxcoin-*-osx-unsigned.tar.gz inputs/', shell=True)
        subprocess.check_call(
            'mv build/out/fittexxcoin-*.tar.gz build/out/fittexxcoin-*.dmg ' + output_dir_osx, shell=True)
        subprocess.check_call(
            'mv build/out/src/fittexxcoin-*.tar.gz ' + output_dir_src, shell=True)
        subprocess.check_call(
            'mv result/fittexxcoin-*-osx-res.yml ' + output_dir_osx, shell=True)
...
def sign():
    ...
    if args.windows:
        ...
        subprocess.check_call('cp inputs/fittexxcoin-' + args.version +
                              '-win-unsigned.tar.gz inputs/fittexxcoin-win-unsigned.tar.gz', shell=True)
        subprocess.check_call(['bin/gbuild', '-i', '--commit', 'signature=' + args.commit,
                               '../fittexxcoin-node/contrib/gitian-descriptors/gitian-win-signer.yml'])
        subprocess.check_call(['bin/gsign', '-p', args.sign_prog, '--signer', args.signer, '--release', args.version + '-win-signed',
                               '--destination', '../gitian.sigs/', '../fittexxcoin-node/contrib/gitian-descriptors/gitian-win-signer.yml'])
        subprocess.check_call(
            'mv build/out/fittexxcoin-*win64-setup.exe ../fittexxcoin-binaries/' + args.version, shell=True)

    if args.macos:
        print('\nSigning ' + args.version + ' MacOS')
        subprocess.check_call('cp inputs/fittexxcoin-' + args.version +
                              '-osx-unsigned.tar.gz inputs/fittexxcoin-osx-unsigned.tar.gz', shell=True)
        subprocess.check_call(['bin/gbuild', '-i', '--commit', 'signature=' + args.commit,
                               '../fittexxcoin-node/contrib/gitian-descriptors/gitian-osx-signer.yml'])
        subprocess.check_call(['bin/gsign', '-p', args.sign_prog, '--signer', args.signer, '--release', args.version + '-osx-signed',
                               '--destination', '../gitian.sigs/', '../fittexxcoin-node/contrib/gitian-descriptors/gitian-osx-signer.yml'])
        subprocess.check_call('mv build/out/fittexxcoin-osx-signed.dmg ../fittexxcoin-binaries/' +
                              args.version + '/fittexxcoin-' + args.version + '-osx.dmg', shell=True)
...
def verify():
    ...
    subprocess.check_call(['bin/gverify', '-v', '-d', '../gitian.sigs/', '-r', args.version +
                           '-linux', '../fittexxcoin-node/contrib/gitian-descriptors/gitian-linux.yml'])
    print('\nVerifying v' + args.version + ' Windows\n')
    subprocess.check_call(['bin/gverify', '-v', '-d', '../gitian.sigs/', '-r', args.version +
                           '-win-unsigned', '../fittexxcoin-node/contrib/gitian-descriptors/gitian-win.yml'])
    print('\nVerifying v' + args.version + ' MacOS\n')
    subprocess.check_call(['bin/gverify', '-v', '-d', '../gitian.sigs/', '-r', args.version +
                           '-osx-unsigned', '../fittexxcoin-node/contrib/gitian-descriptors/gitian-osx.yml'])
    print('\nVerifying v' + args.version + ' Signed Windows\n')
    subprocess.check_call(['bin/gverify', '-v', '-d', '../gitian.sigs/', '-r', args.version +
                           '-win-signed', '../fittexxcoin-node/contrib/gitian-descriptors/gitian-win-signer.yml'])
    print('\nVerifying v' + args.version + ' Signed MacOS\n')
    subprocess.check_call(['bin/gverify', '-v', '-d', '../gitian.sigs/', '-r', args.version +
                           '-osx-signed', '../fittexxcoin-node/contrib/gitian-descriptors/gitian-osx-signer.yml'])
...
def main():
    ...
    parser.add_argument('-u', '--url', dest='url', default='https://gitlab.com/fittexxcoin-node/fittexxcoin-node.git',
    ...
    if not os.path.isdir('gitian-builder/inputs/fittexxcoin'):
        subprocess.check_call(['mkdir', '-p', 'gitian-builder/inputs'])
        os.chdir('gitian-builder/inputs')
        subprocess.check_call(['git', 'clone', args.url, 'fittexxcoin'])

    # Move to fittexxcoin-node folder
    os.chdir(os.path.join(workdir, 'fittexxcoin-node'))
    ...
    # Still in fittexxcoin-node/ ...
    subprocess.check_call(['git', 'fetch'])
    subprocess.check_call(['git', 'checkout', args.commit])

    # Compare our own source code to fittexxcoin-node version of gitian-build.py
    ...
    # Update inputs/fittexxcoin/ to the commit to build
    os.chdir(os.path.join(workdir, 'gitian-builder/inputs/fittexxcoin'))
