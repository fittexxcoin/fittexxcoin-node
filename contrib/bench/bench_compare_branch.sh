...
result_base=$(mktemp "fxxn_bench_compare_tmp.$compare_base.XXXXXX.txt")
result_branch=$(mktemp "fxxn_bench_compare_tmp.$compare_branch.XXXXXX.txt")
...
if [ "$current_branch" = "$compare_base" ]; then
  ...
  ninja bench_fittexxcoin >/dev/null && \
    echo "Benchmarking $compare_base..." && \
    src/bench/bench_fittexxcoin "$filter_arg" > "$result_base" || exit 1
  echo "Building $compare_branch..." >&2
  git checkout "$compare_branch" >&2 && \
    ninja bench_fittexxcoin >/dev/null && \
    echo "Benchmarking $compare_branch" && \
    src/bench/bench_fittexxcoin "$filter_arg" > "$result_branch" || exit 1
