...
cd "${SCRIPT_PATH}"
GIT_REPO="https://${OAUTH_TOKEN}@github.com/fittexxcoin-node/fittexxcoin-node.git"
...
if [ -d "${ASSET_DIR}" ]; then
  # Linux binaries
  ASSET_LIST+=("${ASSET_DIR}/linux/fittexxcoin-node-${VERSION}-aarch64-linux-gnu.tar.gz")
  ASSET_LIST+=("${ASSET_DIR}/linux/fittexxcoin-node-${VERSION}-arm-linux-gnueabihf.tar.gz")
  ASSET_LIST+=("${ASSET_DIR}/linux/fittexxcoin-node-${VERSION}-i686-pc-linux-gnu.tar.gz")
  ASSET_LIST+=("${ASSET_DIR}/linux/fittexxcoin-node-${VERSION}-x86_64-linux-gnu.tar.gz")

  # OSX binaries
  ASSET_LIST+=("${ASSET_DIR}/osx/fittexxcoin-node-${VERSION}-osx-unsigned.dmg")

  # Windows binaries
  ASSET_LIST+=("${ASSET_DIR}/win/fittexxcoin-node-${VERSION}-win32-setup-unsigned.exe")
  ASSET_LIST+=("${ASSET_DIR}/win/fittexxcoin-node-${VERSION}-win64-setup-unsigned.exe")
...
URL="https://api.github.com/repos/fittexxcoin-node/fittexxcoin-node/releases"
...
if [ "${DRY_RUN}" == "true" ]; then
  ...
  UPLOAD_URL="https://uploads.github.com/repos/fittexxcoin-node/fittexxcoin-node/releases/${RELEASE_ID}/assets"
...
echo "Done."
echo "https://github.com/fittexxcoin-node/fittexxcoin-node/releases/tag/${TAG}"
