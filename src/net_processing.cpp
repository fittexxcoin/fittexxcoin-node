...
static bool ProcessMessage(const Config &config, CNode *pfrom,
    ...
    if (msg_type == NetMsgType::TX) {
        ...
        if (!AlreadyHave(inv) &&
                ...
                for (auto mi = itByPrev->second.begin();
                    ...
                    if (AcceptToMemoryPool(config, g_mempool, stateDummy,
                        ...
                        if (!stateDummy.CorruptionPossible()) {
                            ...
                            // https://github.com/fittexxcoin/fittexxcoin/issues/8279
                            // for details.
                            assert(recentRejects);
            ...
            if (!state.CorruptionPossible()) {
                ...
                // malleated. See https://github.com/fittexxcoin/fittexxcoin/issues/8279
                // for details.
                assert(recentRejects);
                recentRejects->insert(tx.GetId());
    ...
    if (msg_type == NetMsgType::DSPROOF) {
        ...
        try {
            ...
            // See: https://gitlab.com/fittexxcoin-node/fittexxcoin-node/-/merge_requests/700#note_417716740
