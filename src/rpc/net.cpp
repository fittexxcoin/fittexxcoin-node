...
static UniValue addnode(const Config &config, const JSONRPCRequest &request) {
    ...
    if (request.fHelp || request.params.size() != 2 || (strCommand != "onetry" && strCommand != "add" && strCommand != "remove")) {
        throw std::runtime_error(RPCHelpMan{
            ...
            RPCResults{},
            RPCExamples{
                HelpExampleCli("addnode", "\"192.168.0.6:7890\" \"onetry\"") +
                HelpExampleRpc("addnode", "\"192.168.0.6:7890\", \"onetry\"")},
        }.ToStringWithResultsAndExamples());
    }
...
static UniValue disconnectnode(const Config &config,
    ...
    if (request.fHelp || request.params.size() == 0 || request.params.size() >= 3) {
        throw std::runtime_error(RPCHelpMan{
            ...
            RPCResults{},
            RPCExamples{
                HelpExampleCli("disconnectnode", "\"192.168.0.6:7890\"") +
                HelpExampleCli("disconnectnode", "\"\" 1") +
                HelpExampleRpc("disconnectnode", "\"192.168.0.6:7890\"") +
    ...
    if (request.fHelp || request.params.size() > 1) {
        throw std::runtime_error(RPCHelpMan{
            ...
            RPCResult{
                ...
                "       {\n"
                "         \"address\" : \"192.168.0.201:7890\",  (string) The fittexxcoin server IP and port we're connected to\n"
    ...
    if (request.fHelp || request.params.size() != 0) {
        throw std::runtime_error(RPCHelpMan{
            ...
            RPCResult{
                ...
                "  \"subversion\": \"/Fittexxcoin Node:x.x.x/\",    (string) the server subversion string\n"
    ...
    if (request.fHelp || request.params.size() > 2) {
        throw std::runtime_error(RPCHelpMan{
            ...
            RPCResult{
                ...
                "}\n"},
            RPCExamples{HelpExampleCli("addpeeraddress", "\"1.2.3.4\" 7890") +
                        HelpExampleRpc("addpeeraddress", "\"1.2.3.4\", 7890")},
        }.ToStringWithResultsAndExamples());
    }
