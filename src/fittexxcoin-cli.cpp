...
static void SetupCliArgs() {
    ...
    gArgs.AddArg(
        ...
        "corresponding -wallet option passed to fittexxcoind)",
        ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);
}
...
static int AppInitRPC(int argc, char *argv[]) {
    ...
    if (argc < 2 || HelpRequested(gArgs) || gArgs.IsArgSet("-version")) {
        ...
        if (!gArgs.IsArgSet("-version")) {
            strUsage += "\n"
                        "Usage:  fittexxcoin-cli [options] <command> [params]  "
                        "Send command to " PACKAGE_NAME "\n"
                        "or:     fittexxcoin-cli [options] -named <command> "
                        "[name=value]...  Send command to " PACKAGE_NAME
                        " (with named arguments)\n"
                        "or:     fittexxcoin-cli [options] help                "
                        "List commands\n"
                        "or:     fittexxcoin-cli [options] help <command>      Get "
                        "help for a command\n";
    ...
    if (response.status == 0) {
        ...
        throw CConnectionFailed(
            strprintf("Could not connect to the server %s:%d%s\n\nMake sure "
                      "the fittexxcoind server is running and that you are "
