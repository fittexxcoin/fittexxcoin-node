...
UniValue importaddress(const Config &config, const JSONRPCRequest &request) {
    ...
    if (request.fHelp || request.params.size() < 1 ||
        ...
        throw std::runtime_error(
            RPCHelpMan{"importaddress",
                ...
                {
                    {"address", RPCArg::Type::STR, /* opt */ false, /* default_val */ "", "The Fittexxcoin address (or hex-encoded script)"},
...

    {
        ...
        if (IsValidDestination(dest)) {
            ...
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY,
                               "Invalid Fittexxcoin address or script");
        }
    }
    if (fRescan) {
        RescanWallet(*pwallet, reserver);
    ...
    if (request.fHelp || request.params.size() != 1) {
        throw std::runtime_error(
            RPCHelpMan{"dumpprivkey",
                ...
                {
                    {"address", RPCArg::Type::STR, /* opt */ false, /* default_val */ "", "The Fittexxcoin address for the private key"},
                }}
                .ToString() +
            "\nResult:\n"
...
UniValue dumpwallet(const Config &config, const JSONRPCRequest &request) {
    ...
    if (request.fHelp || request.params.size() != 1) {
        throw std::runtime_error(
            RPCHelpMan{"dumpwallet",
                ...
                {
                    {"filename", RPCArg::Type::STR, /* opt */ false, /* default_val */ "", "The filename with path (either absolute or relative to fittexxcoind)"},
                }}
                .ToString() +
            "\nResult:\n"
            "{                           (json object)\n"
     ...
     * https://github.com/fittexxcoin/fittexxcoin/issues/9934
    ...
    // produce output
    file << strprintf("# Wallet dump created by Fittexxcoin %s\n", CLIENT_BUILD);
                    ...
                    result.emplace_back(
                        ...
                        JSONRPCError(
                            ...
                            strprintf(
                                ...
                                "(see fittexxcoind log for details) and could be "
