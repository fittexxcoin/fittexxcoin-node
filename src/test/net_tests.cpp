...
BOOST_AUTO_TEST_CASE(caddrdb_read) {
    ...
    BOOST_CHECK(Lookup("250.7.1.1", addr1, 7890, false));
    ...
    CService source;
    BOOST_CHECK(Lookup("252.5.1.1", source, 7890, false));
...
BOOST_AUTO_TEST_CASE(cnetaddr_unserialize_v2) {
    ...
    s << Span{ParseHex("02"                                  // network type (IPv6)
                       "10"                                  // address length
                       "fd6b88c08724ca978112ca1bbdcafac2")}; // address: 0xfd + sha256("fittexxcoin")[0:5] +
                                                             // sha256(name)[0:10]
    s >> addr;
...
BOOST_AUTO_TEST_CASE(test_userAgent) {
    ...
    const std::string versionMessage =
        "/Fittexxcoin Node:" + std::to_string(CLIENT_VERSION_MAJOR) + "." +
    ...
    const std::string versionMessage_strprintf =
        strprintf("/Fittexxcoin Node:%d.%d.%d(EB8.0; %s)/", CLIENT_VERSION_MAJOR, CLIENT_VERSION_MINOR, CLIENT_VERSION_REVISION, uacomment);
