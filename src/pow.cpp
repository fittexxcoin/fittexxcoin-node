...
/**
 * Compute the next required proof of work using the legacy Fittexxcoin difficulty
...
uint32_t GetNextCashWorkRequired(const CBlockIndex *pindexPrev,
    ...
    const uint32_t nHeight = pindexPrev->nHeight + 1;

    // --- PATCH START: Allow early blocks before DAA kicks in ---
    if (nHeight < params.DifficultyAdjustmentInterval()) {
        return pindexPrev->nBits;  // Just use previous difficulty (static)
    }
    // --- PATCH END ---
