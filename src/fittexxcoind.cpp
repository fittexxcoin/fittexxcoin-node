 ...
 *
 * This is the developer documentation of Fittexxcoin Node
 * (https://www.fittexxcoinnode.org/). Fittexxcoin Node is a client for the
 * digital currency called Fittexxcoin (https://www.fittexxcoin.org/), which
 * enables instant payments to anyone, anywhere in the world. Fittexxcoin uses
...
static bool AppInit(int argc, char *argv[]) {
    ...
    // Parameters
    //
    // If Qt is used, parameters/fittexxcoin.conf are parsed in qt/fittexxcoin.cpp's
    // main()
    SetupServerArgs();
    std::string error;
        ...
        for (int i = 1; i < argc; i++) {
            if (!IsSwitchChar(argv[i][0])) {
                fprintf(stderr,
                        ...
                        "see fittexxcoind -h for a list of options.\n",
                        argv[i]);
                return false;
            }
        }

        // -server defaults to true for fittexxcoind but not for the GUI so do this
        // here
...
#endif
            fprintf(stdout, "Fittexxcoin server starting\n");
...
int main(int argc, char *argv[]) {
    ...
    // Connect fittexxcoind signal handlers
    noui_connect();
