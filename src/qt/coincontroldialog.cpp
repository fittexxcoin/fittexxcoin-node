...
void CoinControlDialog::copyAmount() {
    GUIUtil::setClipboard(
        FittexxcoinUnits::removeSpaces(contextMenuItem->text(COLUMN_AMOUNT)));
}
...
void CoinControlDialog::updateLabels(WalletModel *model, QDialog *dialog) {
    ...
    int nDisplayUnit = FittexxcoinUnits::fxx;
    ...
    // Amount
    l2->setText(FittexxcoinUnits::formatWithUnit(nDisplayUnit, nAmount));
    // Fee
    l3->setText(FittexxcoinUnits::formatWithUnit(nDisplayUnit, nPayFee));
    // After Fee
    l4->setText(FittexxcoinUnits::formatWithUnit(nDisplayUnit, nAfterFee));
    // Bytes
    l5->setText(((nBytes > 0) ? ASYMP_UTF8 : "") + QString::number(nBytes));
    // Dust
    l7->setText(fDust ? tr("yes") : tr("no"));
    // Change
    l8->setText(FittexxcoinUnits::formatWithUnit(nDisplayUnit, nChange));
    ...
    for (const auto &coins : model->wallet().listCoins()) {
        ...
        for (const auto &outpair : coins.second) {
            ...
            if (ExtractDestination(out.txout.scriptPubKey, outputAddress, 0 /* no p2sh_32 */)) {
                ...
                // if listMode or change => show Fittexxcoin address. In tree mode,
            ...
            itemOutput->setText(
                COLUMN_AMOUNT,
                FittexxcoinUnits::format(nDisplayUnit, out.txout.nValue));
        ...
        if (treeMode) {
            ...
            itemWalletAddress->setText(
                COLUMN_AMOUNT, FittexxcoinUnits::format(nDisplayUnit, nSum));
