...
#include <qt/fittexxcoinamountfield.h>
...
public:
    explicit AmountSpinBox(QWidget *parent)
        : QAbstractSpinBox(parent), currentUnit(FittexxcoinUnits::fxx),
    ...
    QValidator::State validate(QString &text, int &pos [[maybe_unused]]) const override {
        ...
        QString digits = FittexxcoinUnits::removeSpaces(text);
    ...
    void fixup(QString &input) const override {
        ...
        if (valid) {
            input = FittexxcoinUnits::format(currentUnit, val, false,
    ...
    void setValue(const Amount value) {
        lineEdit()->setText(FittexxcoinUnits::format(
            currentUnit, value, false, FittexxcoinUnits::separatorAlways));
        Q_EMIT valueChanged();
    }
    ...
    QSize minimumSizeHint() const override {
        if (cachedMinimumSizeHint.isEmpty()) {
            ...
            int w = GUIUtil::TextWidth(
                fm, FittexxcoinUnits::format(FittexxcoinUnits::fxx,
                                         MAX_MONEY, false,
                                         FittexxcoinUnits::separatorAlways));
...
private:
    ...
    Amount parse(const QString &text, bool *valid_out = nullptr) const {
        auto val = FittexxcoinUnits::parse(currentUnit, true, text);
...
protected:
    bool event(QEvent *event) override {
        if (event->type() == QEvent::KeyPress || event->type() == QEvent::KeyRelease) {
            ...
            bool preferComma = FittexxcoinUnits::decimalSeparatorIsComma();
...
#include <qt/fittexxcoinamountfield.moc>

FittexxcoinAmountField::FittexxcoinAmountField(QWidget *parent)
    ...
    unit->setModel(new FittexxcoinUnits(this));
    ...
    connect(amount, &AmountSpinBox::valueChanged, this,
            &FittexxcoinAmountField::valueChanged);
    connect(
        unit,
        static_cast<void (QComboBox::*)(int)>(&QComboBox::currentIndexChanged),
        this, &FittexxcoinAmountField::unitChanged);

    // Set default based on configuration
    unitChanged(unit->currentIndex());
}

void FittexxcoinAmountField::clear() {
    amount->clear();
    unit->setCurrentIndex(0);
}

void FittexxcoinAmountField::setEnabled(bool fEnabled) {
    amount->setEnabled(fEnabled);
    unit->setEnabled(fEnabled);
}

bool FittexxcoinAmountField::validate() {
    bool valid = false;
    value(&valid);
    setValid(valid);
    return valid;
...
void FittexxcoinAmountField::setValid(bool valid) {
    if (valid) {
...
bool FittexxcoinAmountField::eventFilter(QObject *object, QEvent *event) {
...
QWidget *FittexxcoinAmountField::setupTafxxain(QWidget *prev) {
    QWidget::setTabOrder(prev, amount);
    QWidget::setTabOrder(amount, unit);
    return unit;
}

Amount FittexxcoinAmountField::value(bool *valid_out) const {
    return amount->value(valid_out);
}

void FittexxcoinAmountField::setValue(const Amount value) {
    amount->setValue(value);
}

void FittexxcoinAmountField::setReadOnly(bool fReadOnly) {
    amount->setReadOnly(fReadOnly);
}

void FittexxcoinAmountField::unitChanged(int idx) {
    // Use description tooltip for current unit for the combobox
    unit->setToolTip(unit->itemData(idx, Qt::ToolTipRole).toString());

    // Determine new unit ID
    int newUnit = unit->itemData(idx, FittexxcoinUnits::UnitRole).toInt();

    amount->setDisplayUnit(newUnit);
}

void FittexxcoinAmountField::setDisplayUnit(int newUnit) {
    unit->setValue(newUnit);
}

void FittexxcoinAmountField::setSingleStep(const Amount step) {
