...
const std::string FittexxcoinGUI::DEFAULT_UIPLATFORM =
#if defined(Q_OS_MAC)
    "macosx"
...
FittexxcoinGUI::FittexxcoinGUI(interfaces::Node &node, const Config *configIn,
    ...
    if (enableWallet) {
        connect(walletFrame, &WalletFrame::requestedSyncWarningInfo, this,
                &FittexxcoinGUI::showModalOverlay);
...
void FittexxcoinGUI::createActions() {
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Fittexxcoin address"));
    ...
    connect(overviewAction, &QAction::triggered, this,
            &FittexxcoinGUI::gotoOverviewPage);
    ...
    connect(receiveCoinsAction, &QAction::triggered, this,
            &FittexxcoinGUI::gotoReceiveCoinsPage);
    ...
    signMessageAction->setStatusTip(
        tr("Sign messages with your Fittexxcoin addresses to prove you own them"));
    verifyMessageAction =
        new QAction(platformStyle->TextColorIcon(":/icons/verify"),
                    tr("&Verify Message..."), this);
    verifyMessageAction->setStatusTip(
        tr("Verify messages to ensure they were signed with specified Fittexxcoin addresses"));
    ...
    if (walletFrame) {
        ...
        connect(openAction, &QAction::triggered, this,
                &FittexxcoinGUI::openClicked);
    }
#endif // ENABLE_WALLET

    connect(new QShortcut(QKeySequence(Qt::CTRL + Qt::SHIFT + Qt::Key_C), this),
            &QShortcut::activated, this,
            &FittexxcoinGUI::showDebugWindowActivateConsole);
    connect(new QShortcut(QKeySequence(Qt::CTRL + Qt::SHIFT + Qt::Key_D), this),
            &QShortcut::activated, this, &FittexxcoinGUI::showDebugWindow);
}

void FittexxcoinGUI::createMenuBar() {
#ifdef Q_OS_MAC
...
void FittexxcoinGUI::createToolBars() {
    if (walletFrame) {
        ...
        connect(m_wallet_selector,
                ...
                this, &FittexxcoinGUI::setCurrentWalletBySelectorIndex);
...
void FittexxcoinGUI::setClientModel(ClientModel *_clientModel) {
    ...
    if (_clientModel) {
        ...
        connect(_clientModel, &ClientModel::numConnectionsChanged, this,
                &FittexxcoinGUI::setNumConnections);
        connect(_clientModel, &ClientModel::networkActiveChanged, this,
                &FittexxcoinGUI::setNetworkActive);
        ...
        if (optionsModel && trayIcon) {
            ...
            connect(optionsModel, &OptionsModel::hideTrayIconChanged, this,
                    &FittexxcoinGUI::setTrayIconVisible);
...
#ifdef ENABLE_WALLET
void FittexxcoinGUI::setWalletController(WalletController *wallet_controller) {
...
void FittexxcoinGUI::addWallet(WalletModel *walletModel) {
    if (!walletFrame) {
        return;
    }
...
void FittexxcoinGUI::removeWallet(WalletModel *walletModel) {
    if (!walletFrame) {
        return;
    }
...
void FittexxcoinGUI::setCurrentWallet(WalletModel *wallet_model) {
    if (!walletFrame) {
        return;
    }
...
void FittexxcoinGUI::setCurrentWalletBySelectorIndex(int index) {
...
void FittexxcoinGUI::removeAllWallets() {
    if (!walletFrame) {
        return;
    }
    setWalletActionsEnabled(false);
...
void FittexxcoinGUI::setWalletActionsEnabled(bool enabled) {
...
void FittexxcoinGUI::createTrayIcon() {
...
void FittexxcoinGUI::createTrayIconMenu() {
#ifndef Q_OS_MAC
...
#ifndef Q_OS_MAC
void FittexxcoinGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason) {
...
#else
void FittexxcoinGUI::macosDockIconActivated() {
    show();
    activateWindow();
}
#endif

void FittexxcoinGUI::optionsClicked() {
    openOptionsDialogWithTab(OptionsDialog::TAB_MAIN);
}

void FittexxcoinGUI::aboutClicked() {
    if (!clientModel) {
        return;
    }
...
void FittexxcoinGUI::showDebugWindow() {
    GUIUtil::bringToFront(rpcConsole);
    Q_EMIT consoleShown(rpcConsole);
}

void FittexxcoinGUI::showDebugWindowActivateConsole() {
    rpcConsole->setTabFocus(RPCConsole::TAB_CONSOLE);
    showDebugWindow();
}

void FittexxcoinGUI::showHelpMessageClicked() {
    helpMessageDialog->show();
}

#ifdef ENABLE_WALLET
void FittexxcoinGUI::openClicked() {
...
void FittexxcoinGUI::gotoOverviewPage() {
    overviewAction->setChecked(true);
    if (walletFrame) {
...
void FittexxcoinGUI::gotoHistoryPage() {
...
void FittexxcoinGUI::gotoReceiveCoinsPage() {
...
void FittexxcoinGUI::gotoSendCoinsPage(QString addr) {
...
void FittexxcoinGUI::gotoSignMessageTab(QString addr) {
    if (walletFrame) {
        walletFrame->gotoSignMessageTab(addr);
    }
}

void FittexxcoinGUI::gotoVerifyMessageTab(QString addr) {
    if (walletFrame) {
...
void FittexxcoinGUI::updateNetworkState() {
    ...
    if (m_node.getNetworkActive()) {
        tooltip = tr("%n active connection(s) to Fittexxcoin network", "", count) +
...
void FittexxcoinGUI::setNumConnections(int count) {
    updateNetworkState();
}

void FittexxcoinGUI::setNetworkActive(bool networkActive) {
    updateNetworkState();
}

void FittexxcoinGUI::updateHeadersSyncProgressLabel() {
...
void FittexxcoinGUI::openOptionsDialogWithTab(OptionsDialog::Tab tab) {
...
void FittexxcoinGUI::setNumBlocks(int count, const QDateTime &blockDate, const QString &,
...
void FittexxcoinGUI::message(const QString &title, const QString &message,
                         unsigned int style, bool *ret) {
    // default title
    QString strTitle = tr("Fittexxcoin");
...
void FittexxcoinGUI::changeEvent(QEvent *e) {
    ...
    if (e->type() == QEvent::WindowStateChange) {
        if (clientModel && clientModel->getOptionsModel() &&
            ...
            if (!(wsevt->oldState() & Qt::WindowMinimized) && isMinimized()) {
                QTimer::singleShot(0, this, &FittexxcoinGUI::hide);
                e->ignore();
            } else if ((wsevt->oldState() & Qt::WindowMinimized) &&
                       !isMinimized()) {
                QTimer::singleShot(0, this, &FittexxcoinGUI::show);
                e->ignore();
            }
        }
    }
...
void FittexxcoinGUI::closeEvent(QCloseEvent *event) {
#ifndef Q_OS_MAC // Ignored on Mac
    if (clientModel && clientModel->getOptionsModel()) {
...
void FittexxcoinGUI::showEvent(QShowEvent *event) {
...
#ifdef ENABLE_WALLET
void FittexxcoinGUI::incomingTransaction(const QString &date, int unit,
    ...
    QString msg = tr("Date: %1\n").arg(date) +
                  tr("Amount: %1\n")
                      .arg(FittexxcoinUnits::formatWithUnit(unit, amount, true));
...
void FittexxcoinGUI::dragEnterEvent(QDragEnterEvent *event) {
    // Accept only URIs
...
void FittexxcoinGUI::dropEvent(QDropEvent *event) {
    if (event->mimeData()->hasUrls()) {
...
bool FittexxcoinGUI::eventFilter(QObject *object, QEvent *event) {
...
#ifdef ENABLE_WALLET
bool FittexxcoinGUI::handlePaymentRequest(const SendCoinsRecipient &recipient) {
    // URI has to be valid
...
void FittexxcoinGUI::setHDStatus(int hdEnabled) {
...
void FittexxcoinGUI::setEncryptionStatus(int status) {
    switch (status) {
...
void FittexxcoinGUI::updateWalletStatus() {
    if (!walletFrame) {
        return;
    }
...
void FittexxcoinGUI::updateProxyIcon() {
    std::string ip_port;
...
void FittexxcoinGUI::updateWindowTitle() {
...
void FittexxcoinGUI::showNormalIfMinimized(bool fToggleHidden) {
    if (!clientModel) {
        return;
    }
...
void FittexxcoinGUI::toggleHidden() {
    showNormalIfMinimized(true);
}

void FittexxcoinGUI::detectShutdown() {
...
void FittexxcoinGUI::showProgress(const QString &title, int nProgress) {
    if (nProgress == 0) {
...
void FittexxcoinGUI::setTrayIconVisible(bool fHideTrayIcon) {
    if (trayIcon) {
        trayIcon->setVisible(!fHideTrayIcon);
    }
}

void FittexxcoinGUI::showModalOverlay() {
    if (modalOverlay &&
...
void FittexxcoinGUI::setWindowActionsEnabled(bool enabled) {
...
static bool ThreadSafeMessageBox(FittexxcoinGUI *gui, const std::string &message,
...
void FittexxcoinGUI::subscribeToCoreSignals() {
...
void FittexxcoinGUI::unsubscribeFromCoreSignals() {
...
UnitDisplayStatusBarControl::UnitDisplayStatusBarControl(
    ...
    QList<FittexxcoinUnits::Unit> units = FittexxcoinUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const FittexxcoinUnits::Unit unit : units) {
        max_width = qMax(max_width, GUIUtil::TextWidth(fm, FittexxcoinUnits::ticker(unit)));
    }
...
void UnitDisplayStatusBarControl::createContextMenu() {
    ...
    for (const FittexxcoinUnits::Unit u : FittexxcoinUnits::availableUnits()) {
        QAction *menuAction = new QAction(FittexxcoinUnits::ticker(u), this);
        menuAction->setStatusTip(tr("Change unit to %1").arg(FittexxcoinUnits::description(u)));
...
void UnitDisplayStatusBarControl::updateDisplayUnit(int newUnits) {
    setText(FittexxcoinUnits::ticker(newUnits));
}
