...
TxId SendCoins(CWallet &wallet, SendCoinsDialog &sendCoinsDialog,
    ...
    entry->findChild<FittexxcoinAmountField *>("payAmount")->setValue(amount);
    TxId txid;
...
//
//     src/qt/test/test_fittexxcoin-qt -platform xcb      # Linux
//     src/qt/test/test_fittexxcoin-qt -platform windows  # Windows
//     src/qt/test/test_fittexxcoin-qt -platform cocoa    # macOS
void TestGUI() {
    ...
    QString balanceComparison = FittexxcoinUnits::formatWithUnit(
        unit, balance, false, FittexxcoinUnits::separatorAlways);
    ...
    // Amount input
    FittexxcoinAmountField *amountInput =
        receiveCoinsDialog.findChild<FittexxcoinAmountField *>("reqAmount");
    ...
    for (QWidget *widget : QApplication::topLevelWidgets()) {
        if (widget->inherits("ReceiveRequestDialog")) {
            ...
            QVERIFY(paymentTextList.at(1).indexOf(QString("URI: fxxreg:")) !=
                    -1);
...
void WalletTests::walletTests() {
    ...
    if (QApplication::platformName() == "minimal") {
        ...
        QWARN("Skipping WalletTests on mac build with 'minimal' platform set "
              "due to Qt bugs. To run AppTests, invoke with 'test_fittexxcoin-qt "
