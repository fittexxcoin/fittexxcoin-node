...
const int FITTEXXCOIN_IPC_CONNECT_TIMEOUT = 1000; // milliseconds
#ifdef ENABLE_BIP70
// BIP70 payment protocol messages
const char *BIP70_MESSAGE_PAYMENTACK = "PaymentACK";
const char *BIP70_MESSAGE_PAYMENTREQUEST = "PaymentRequest";
// BIP71 payment protocol media types
const char *BIP71_MIMETYPE_PAYMENT = "application/fittexxcoin-payment";
const char *BIP71_MIMETYPE_PAYMENTACK = "application/fittexxcoin-paymentack";
const char *BIP71_MIMETYPE_PAYMENTREQUEST =
    "application/fittexxcoin-paymentrequest";
#endif
...
static std::string ipcParseURI(const QString &arg, const CChainParams &params,
    ...
    if (!GUIUtil::parseFittexxcoinURI(scheme, arg, &r)) {
        return {};
    }
...
bool PaymentServer::ipcSendCommandLine() {
    ...
    for (const QString &r : savedPaymentRequests) {
        ...
        if (!socket->waitForConnected(FITTEXXCOIN_IPC_CONNECT_TIMEOUT)) {
            delete socket;
            socket = nullptr;
            return false;
        }
        ...
        socket->write(block);
        socket->flush();
        socket->waitForBytesWritten(FITTEXXCOIN_IPC_CONNECT_TIMEOUT);
...
// clang-format on
{
    ...
    // on Mac: sent when you click fittexxcoin: links
...
//
// OSX-specific way of handling fittexxcoin: URIs and PaymentRequest mime types.
...
bool PaymentServer::handleURI(const CChainParams &params, const QString &s) {
    ...
    if (GUIUtil::parseFittexxcoinURI(scheme, s, &recipient)) {
        ...
        Q_EMIT message(
            ...
            tr("URI cannot be parsed! This can be caused by an invalid "
               "Fittexxcoin address or malformed URI parameters."),
            CClientUIInterface::ICON_WARNING);
    }
...
void PaymentServer::initNetManager() {
    ...
    // netManager is used to fetch paymentrequests given in fittexxcoin: URIs
...
bool PaymentServer::processPaymentRequest(const PaymentRequestPlus &request,
    ...
    for (const std::pair<CScript, Amount> &sendingTo : sendingTos) {
        ...
        if (ExtractDestination(sendingTo.first, dest, 0 /* no p2sh_32 */)) {
            ...
            // Unauthenticated payment requests to custom Fittexxcoin addresses are
        ...
        // Fittexxcoin amounts are stored as (optional) uint64 in the protobuf
