...
Intro::Intro(QWidget *parent, uint64_t blockchain_size,
    ...
    ui->sizeWarningLabel->setText(
        tr("%1 will download and store a copy of the Fittexxcoin block chain.")
            .arg(PACKAGE_NAME) +
...
bool Intro::pickDataDirectory(interfaces::Node &node) {
    ...
    if (!fs::exists(GUIUtil::qstringToBoostPath(dataDir)) ||
        ...
        intro.setWindowIcon(QIcon(":icons/fittexxcoin"));
     ...
     * to
     * override -datadir in the fittexxcoin.conf file in the default data directory
     * (to be consistent with fittexxcoind behavior)
     */
    if (dataDir != getDefaultDataDirectory()) {
