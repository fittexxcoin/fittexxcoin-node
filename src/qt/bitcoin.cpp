...
static void initTranslations(QTranslator &qtTranslatorBase,
    ...
    // Load e.g. fittexxcoin_de.qm (shortcut "de" needs to be defined in
    // fittexxcoin.qrc)
    if (translatorBase.load(lang, ":/translations/")) {
        QApplication::installTranslator(&translatorBase);
    }

    // Load e.g. fittexxcoin_de_DE.qm (shortcut "de_DE" needs to be defined in
    // fittexxcoin.qrc)
...
FittexxcoinNode::FittexxcoinNode(interfaces::Node &node) : QObject(), m_node(node) {}

void FittexxcoinNode::handleRunawayException(const std::exception *e) {
    PrintExceptionContinue(e, "Runaway exception");
    Q_EMIT runawayException(QString::fromStdString(m_node.getWarnings("gui")));
}

void FittexxcoinNode::initialize(Config *config, RPCServer *rpcServer,
...
FittexxcoinApplication::FittexxcoinApplication(interfaces::Node &node, int &argc,
                                       char **argv)
...
void FittexxcoinApplication::setupPlatformStyle() {
    // UI per-platform customization
    // This must be done inside the FittexxcoinApplication constructor, or after it,
    // because PlatformStyle::instantiate requires a QApplication.
    std::string platformName;
    platformName = gArgs.GetArg("-uiplatform", FittexxcoinGUI::DEFAULT_UIPLATFORM);
    platformStyle =
...
FittexxcoinApplication::~FittexxcoinApplication() {
    if (coreThread) {
...
#ifdef ENABLE_WALLET
void FittexxcoinApplication::createPaymentServer() {
    paymentServer = new PaymentServer(this);
}
#endif

void FittexxcoinApplication::createOptionsModel(bool resetSettings) {
    optionsModel = new OptionsModel(m_node, nullptr, resetSettings);
}

void FittexxcoinApplication::createWindow(const Config *config,
                                      const NetworkStyle *networkStyle) {
    window =
        new FittexxcoinGUI(m_node, config, platformStyle, networkStyle, nullptr);

    pollShutdownTimer = new QTimer(window);
    connect(pollShutdownTimer, &QTimer::timeout, window,
            &FittexxcoinGUI::detectShutdown);
}

void FittexxcoinApplication::createSplashScreen(const NetworkStyle *networkStyle) {
    ...
    // happens.
    splash->show();
    connect(this, &FittexxcoinApplication::splashFinished, splash,
            &SplashScreen::slotFinish);
    connect(this, &FittexxcoinApplication::requestedShutdown, splash,
            &QWidget::close);
}

bool FittexxcoinApplication::baseInitialize(Config &config) {
    return m_node.baseInitialize(config);
}

void FittexxcoinApplication::startThread() {
    if (coreThread) {
        return;
    }
    coreThread = new QThread(this);
    FittexxcoinNode *executor = new FittexxcoinNode(m_node);
    executor->moveToThread(coreThread);

    /*  communication to and from thread */
    connect(executor, &FittexxcoinNode::initializeResult, this,
            &FittexxcoinApplication::initializeResult);
    connect(executor, &FittexxcoinNode::shutdownResult, this,
            &FittexxcoinApplication::shutdownResult);
    connect(executor, &FittexxcoinNode::runawayException, this,
            &FittexxcoinApplication::handleRunawayException);
    ...
    connect(this, &FittexxcoinApplication::requestedInitialize, executor,
            &FittexxcoinNode::initialize);

    connect(this, &FittexxcoinApplication::requestedShutdown, executor,
            &FittexxcoinNode::shutdown);
    /*  make sure executor object is deleted in its own thread */
    connect(this, &FittexxcoinApplication::stopThread, executor,
            &QObject::deleteLater);
    connect(this, &FittexxcoinApplication::stopThread, coreThread, &QThread::quit);

    coreThread->start();
}

void FittexxcoinApplication::parameterSetup() {
...
void FittexxcoinApplication::requestInitialize(
...
void FittexxcoinApplication::requestShutdown(Config &config) {
...
void FittexxcoinApplication::initializeResult(bool success) {
    ...
    // fittexxcoin: URIs or payment requests:
    if (paymentServer) {
        connect(paymentServer, &PaymentServer::receivedPaymentRequest, window,
                &FittexxcoinGUI::handlePaymentRequest);
        connect(window, &FittexxcoinGUI::receivedURI, paymentServer,
...
void FittexxcoinApplication::shutdownResult() {
    // Exit second main loop invocation after shutdown finished.
    quit();
}

void FittexxcoinApplication::handleRunawayException(const QString &message) {
    QMessageBox::critical(
        nullptr, "Runaway exception",
        FittexxcoinGUI::tr("A fatal error occurred. Fittexxcoin can no longer continue "
                       "safely and will quit.") +
            QString("\n\n") + message);
    ::exit(EXIT_FAILURE);
}

WId FittexxcoinApplication::getMainWinId() const {
    if (!window) {
        return 0;
    }
...
static void SetupUIArgs() {
    ...
    gArgs.AddArg("-uiplatform",
                 strprintf("Select platform to customize UI for (one of "
                           ...
                           FittexxcoinGUI::DEFAULT_UIPLATFORM),
                 ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::GUI);
}

#ifndef FITTEXXCOIN_QT_TEST
...
int GuiMain(int argc, char *argv[]) {
    ...
    /// 0. Parse fittexxcoin-qt command-line options.
    ...
    /// configuration)
    Q_INIT_RESOURCE(fittexxcoin);
    Q_INIT_RESOURCE(fittexxcoin_locale);
    ...
    int argcQt = 1;
    FittexxcoinApplication app(*node, argcQt, argv);
    ...
    // fittexxcoin: links repeatedly have their payment requests routed to this
    // process:
