 ...
 */
static const char *FITTEXXCOIN_PID_FILENAME = "fittexxcoind.pid";

static fs::path GetPidFile() {
    return AbsPathForConfigVal(
        fs::path(gArgs.GetArg("-pid", FITTEXXCOIN_PID_FILENAME)));
}
 ...
 * is shared between fittexxcoind, and fittexxcoin-qt and non-server tools.
 */
...
void SetupServerArgs() {
    ...
    gArgs.AddArg(
        ...
        strprintf("Activation time of the November 2020 Fittexxcoin Network Upgrade (<n> seconds since epoch, "
    ...
    gArgs.AddArg(
        ...
        strprintf("Activation time of the May 2023 Fittexxcoin Network Upgrade (<n> seconds since epoch, "
                  "default: %d, chipnet: %d)",
    ...
    gArgs.AddArg(
        ...
        strprintf("Activation time of the tentative May 2024 Fittexxcoin Network Upgrade (<n> seconds since epoch, "
                  "default: %d, chipnet: %d)",
...
std::string LicenseInfo() {
    constexpr auto URL_SOURCE_CODE = "<https://gitlab.com/fittexxcoin-node/fittexxcoin-node>";
    constexpr auto URL_WEBSITE = "<https://fittexxcoinnode.org>";
...
/** Sanity checks
 *  Ensure that Fittexxcoin is running in a usable environment with all
...
bool AppInitParameterInteraction(Config &config) {
    ...
    // Signal Fittexxcoin support.
    // TODO: remove some time after the hardfork when no longer needed
    // to differentiate the network nodes.
    nLocalServices = ServiceFlags(nLocalServices | NODE_FITTEXXCOIN_CASH);
...
static bool LockDataDirectory(bool probeOnly) {
    // Make sure only a single Fittexxcoin process is using the data directory.
...
bool AppInitMain(Config &config, RPCServer &rpcServer,
    ...
    if (gArgs.IsArgSet("-datadir") &&
        ...
        LogPrintf("Warning: relative datadir option '%s' specified, which will "
                  ...
                  "'%s'. This is fragile, because if fittexxcoin is started in the "
                  "future from a different location, it will be unable to "
                  "locate the current data files. There could also be data "
                  "loss if fittexxcoin is started while in a temporary "
                  "directory.\n",
