...
#define FITTEXXCOIN_SEED_NONCE 0x0539a019ca550825ULL
...
void CSeederNode::PushVersion() {
    ...
    const uint64_t nLocalNonce = FITTEXXCOIN_SEED_NONCE;
    const int64_t nLocalServices = 0;
    const CAddress me(CService{}, ServiceFlags(NODE_NETWORK | NODE_FITTEXXCOIN_CASH));
    BeginMessage(NetMsgType::VERSION);
    const int nBestHeight = GetRequireHeight();
    const std::string ver = strprintf("/fittexxcoin-seeder:%i.%i.%i/",
...
CSeederNode::CSeederNode(const CService &ip, std::vector<CAddress> *vAddrIn)
    : sock(INVALID_SOCKET), vSend(SER_NETWORK, 0), vRecv(SER_NETWORK, 0),
      ...
      you(ip, ServiceFlags(NODE_NETWORK | NODE_FITTEXXCOIN_CASH)) {
