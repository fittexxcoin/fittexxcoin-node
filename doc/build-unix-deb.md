...
`-DBUILD_FITTEXXCOIN_ZMQ=OFF`, or `-DENABLE_MAN=OFF` on the `cmake` command line.
...
You will find the `fittexxcoind`, `fittexxcoin-cli`, `fittexxcoin-tx` (and optionally
`fittexxcoin-qt`) binaries in `/build/src/(qt)`.
