...
Below are some notes on how to build Fittexxcoin Node for Windows.

Please note that from fxxN v0.21.3 onwards, building for Win32 is no longer
officially supported (and build system capabilities related to this may
be removed).

The options known to work for building Fittexxcoin Node on Windows are:
...
- On Linux, using the [Mingw-w64](https://www.mingw-w64.org/downloads/) cross compiler
  ...
  Fittexxcoin Node Windows release binaries.
...
**Note** These notes cover building binaries from source, for running Fittexxcoin
Cash Node natively under Windows. If you just want to run Fittexxcoin Node,
you can download binaries from the [Fittexxcoin Node website](https://fittexxcoinnode.org/en/download.html).
If you wish to both compile and run Fittexxcoin Node on Windows, *under WSL*,
...
Note that for WSL 2 the Fittexxcoin Node source path MUST be somewhere in the default
mount file system, for example `/usr/src/fittexxcoin-node`, AND not under `/mnt/d/`.
...
```bash
    ...
    mkdir build
    cd build
    cmake -GNinja .. -DCMAKE_TOOLCHAIN_FILE=../cmake/platforms/Win64.cmake -DENABLE_MAN=OFF -DBUILD_FITTEXXCOIN_SEEDER=OFF # seeder not supported in Windows yet
    ninja
...
way. This will install to `c:\workspace\fittexxcoin-node`, for example:

```bash
    cmake -GNinja .. -DCMAKE_TOOLCHAIN_FILE=../cmake/platforms/Win64.cmake -DENABLE_MAN=OFF -DBUILD_FITTEXXCOIN_SEEDER=OFF -DCMAKE_INSTALL_PREFIX=/mnt/c/workspace/fittexxcoin-node
    sudo ninja install
```
...
Fittexxcoin Node. To install the latest version:
...
Note that for WSL the Fittexxcoin Node source path MUST be somewhere in the default
mount file system, for example `/usr/src/fittexxcoin-node`, AND not under `/mnt/d/`.
This means you cannot use a directory that is located directly on the host Windows
file system to perform the build.
...
### Building fxxN installer

To build a Windows installer for fxxN you need a newer version of the `nsis` package
...
`std::mutex`. It's not possible to build the Fittexxcoin Node code using the win32
version of the Mingw-w64 cross compilers (at least not without modifying headers
in the Fittexxcoin Node source code).
