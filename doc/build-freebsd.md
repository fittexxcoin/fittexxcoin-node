...
This guide describes how to build fittexxcoind and command-line utilities on FreeBSD.
...
If not installed, ZeroMQ support should be disabled by passing `-DBUILD_FITTEXXCOIN_ZMQ=OFF`
to `cmake`.
...
If you also want to build the GUI client `fittexxcoin-qt` Qt 5 is necessary.
...
mkdir build
cd build
cmake -GNinja -DBUILD_FITTEXXCOIN_QT=OFF -DBUILD_FITTEXXCOIN_WALLET=OFF ..
ninja
ninja check # recommended
```
...
ninja
ninja check # recommended
ninja test_fittexxcoin-qt # recommended
```
