# XVersionMessage: fxx node extended version and configuration fields
...
Using the `version` message in the Fittexxcoin protocol (for more details,
see [https://en.fittexxcoin.it/wiki/Protocol_documentation#version](https://en.fittexxcoin.it/wiki/Protocol_documentation#version)),
...
encoded in the standard Fittexxcoin message frame[2].

The `xmap` is encoded using Fittexxcoin's usual network de-/serialization
...
by the creator of the key, however, Fittexxcoin P2P network serialization
is recommended since it is also used to encode/decode most of the other
the messages in the Fittexxcoin protocol.
...
A value of `u64c` type is encoded in Fittexxcoin's message serialization as a
...
### Fittexxcoin Unlimited

In the Fittexxcoin Unlimited reference implementation, the `xversion`
