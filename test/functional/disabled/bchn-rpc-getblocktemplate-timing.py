...
'''
fxxn-rpc-getblocktemplate-timing -- test that -maxgbttime and -maxinitialgbttime command-line options work
'''
...
class GBTTimingTest(FittexxcoinTestFramework):
    ...
    def generate_spam(self, value, txcount):
        def helper(node, count, rate=100):
            ...
            for i in range(0, count):
                now = time.time()
                # sleeping for 5 ms per tx ensures that fittexxcoind doesn't get starved for cs_main time and allows
                ...
                try:
                    ...
                    print("Warning: this fittexxcoind appears to not support the 'fast' argument for sendtoaddress")
