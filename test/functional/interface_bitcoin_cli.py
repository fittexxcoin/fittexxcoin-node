...
class TestFittexxcoinCli(FittexxcoinTestFramework):
    ...
    def run_test(self):
        ...
        assert "Fittexxcoin Node RPC client version" in cli_response

        self.log.info(
            "Compare responses from gewalletinfo RPC and `fittexxcoin-cli getwalletinfo`")
        cli_response = self.nodes[0].cli.getwalletinfo()
        rpc_response = self.nodes[0].getwalletinfo()
        assert_equal(cli_response, rpc_response)

        self.log.info(
            "Compare responses from getblockchaininfo RPC and `fittexxcoin-cli getblockchaininfo`")
        ...
        self.log.info(
            "Compare responses from `fittexxcoin-cli -getinfo` and the RPCs data is retrieved from.")
