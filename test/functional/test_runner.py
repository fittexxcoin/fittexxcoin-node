...
`test/functional/test_framework/test_framework.py:FittexxcoinTestFramework.main`.
...
TEST_PARAMS = {
    ...
    "fxxn-txbroadcastinterval.py": [["--testoutbound"]],
...
def main():
    ...
    parser.add_argument('--testsuitename', '-n', default='Fittexxcoin Node functional tests',
    ...
    tmpdir = os.path.join("{}", "fittexxcoin_test_runner_{:%Y%m%d_%H%M%S}").format(
    ...
    enable_fittexxcoind = config["components"].getboolean("ENABLE_FITTEXXCOIND")

    if config["environment"]["EXEEXT"] == ".exe" and not args.force:
        # https://github.com/fittexxcoin/fittexxcoin/commit/d52802551752140cf41f0d9a225a43e84404d3e9
        # https://github.com/fittexxcoin/fittexxcoin/pull/5677#issuecomment-136646964
        print(
            "Tests currently disabled on Windows by default. Use --force option to enable")
        sys.exit(0)

    if not enable_fittexxcoind:
...
def run_tests(test_list, build_dir, tests_dir, junitoutput, tmpdir, num_jobs, test_suite_name,
    ...
    # Warn if fittexxcoind is already running (unix only)
    try:
        pidofOutput = subprocess.check_output(["pidof", "fittexxcoind"])
        if pidofOutput is not None and pidofOutput != b'':
            print("{}WARNING!{} There is already a fittexxcoind process running on this system. Tests may fail unexpectedly due to resource contention!".format(
                BOLD[1], BOLD[0]))
...
def execute_test_processes(
    ...
    # In case there is a graveyard of zombie fittexxcoinds, we can apply a
...
def check_script_prefixes(all_scripts):
    ...
    good_prefixes_re = re.compile(
        "(fxxn[_-])?(example|feature|interface|mempool|mining|p2p|rpc|wallet|tool|ui)[_-]")
    bad_script_names = [
...
class RPCCoverage():
    ...
    commands per `fittexxcoin-cli help` (`rpc_interface.txt`).
