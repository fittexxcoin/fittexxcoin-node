...
class SegWit(NamedTuple):
    addr: str  # fittexxcoin address
    spk: CScript  # scriptPubKey
    rs: bytes  # redeemScript


class P2SH32Test(FittexxcoinTestFramework):
    ...
    def run_test(self):
        ...
        p2pkh_addr = cashaddr.encode("fxxreg", cashaddr.PUBKEY_TYPE, hash160(pub_key))
        self.addr_cashaddr = cashaddr.encode("fxxreg", cashaddr.SCRIPT_TYPE, hash256(redeem_script))
        ...
        p2sh32_addr_false = cashaddr.encode("fxxreg", cashaddr.SCRIPT_TYPE, hash256(redeem_script_op_false))
        p2sh32_addr_true = cashaddr.encode("fxxreg", cashaddr.SCRIPT_TYPE, hash256(redeem_script_op_true))
        ...
        for txn_segwit in txns_spend_segwit[1]:
            ...
            if 5 >= len(script_sig) == script_sig[2] + 3:
                ...
                # which violates the fxx "clean stack" rule
                reject_reason = 'non-mandatory-script-verify-flag (Extra items left on stack after execution)'
    ...
    def create_segwit_address_spk_and_redeem_script(case: int) -> SegWit:
        ...
        address = cashaddr.encode("fxxreg", cashaddr.SCRIPT_TYPE, script_hash)
