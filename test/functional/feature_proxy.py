...
"""Test fittexxcoind with different proxy configuration.

Test plan:
- Start fittexxcoind's with different proxy configurations
- Use addnode to initiate connections
- Verify that proxies are connected to, and the right connection command is given
- Proxy configurations to test on fittexxcoind side:
...
- Create fittexxcoinds that connect to them
- Manipulate the fittexxcoinds using addnode (onetry) an observe effects
...
class ProxyTest(FittexxcoinTestFramework):
    ...
    def node_test(self, node, proxies, auth, test_onion=True):
        ...
        # Note: fittexxcoind's SOCKS5 implementation only sends atyp DOMAINNAME,
        ...
        if self.have_ipv6:
            ...
            assert isinstance(cmd, Socks5Command)
            # Note: fittexxcoind's SOCKS5 implementation only sends atyp
        ...
        if test_onion:
            ...
            node.addnode("fittexxcoinostk4e4re.onion:7890", "onetry")
            cmd = proxies[2].queue.get()
            assert isinstance(cmd, Socks5Command)
            assert_equal(cmd.atyp, AddressType.DOMAINNAME)
            assert_equal(cmd.addr, b"fittexxcoinostk4e4re.onion")
            assert_equal(cmd.port, 7890)
            if not auth:
                assert_equal(cmd.username, None)
                assert_equal(cmd.password, None)
            rv.append(cmd)
        ...
        node.addnode("node.noumenon:7890", "onetry")
