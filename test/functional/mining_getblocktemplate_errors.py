#!/usr/bin/env python3
# Copyright (c) 2020 The Fittexxcoin Node developers
...
class GetBlockTemplateTest(FittexxcoinTestFramework):
    ...
    def run_test(self):
        ...
        # When fittexxcoind is just started it's in IBD
        assert_raises_rpc_error(-10, "Fittexxcoin is downloading blocks...", self.nodes[0].getblocktemplate)
        ...
        assert_raises_rpc_error(-9, "Fittexxcoin is not connected!", self.nodes[0].getblocktemplate)
        assert_raises_rpc_error(-9, "Fittexxcoin is not connected!", self.nodes[1].getblocktemplate)
