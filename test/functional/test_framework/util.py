...
def assert_fee_amount(fee, tx_size, fee_per_kB, wiggleroom=2):
    ...
    if fee < (tx_size - wiggleroom) * fee_per_kB / 1000:
        raise AssertionError(
            "Fee of {} fxx too low! (Should be {} fxx)".format(str(fee), str(target_fee)))
    if fee > (tx_size + wiggleroom) * fee_per_kB / 1000:
        raise AssertionError(
            "Fee of {} fxx too high! (Should be {} fxx)".format(str(fee), str(target_fee)))
...
def check_json_precision():
    """Make sure json library being used does not lose precision converting fxx values"""
...
def initialize_datadir(dirname, n, chain):
    ...
    with open(os.path.join(datadir, "fittexxcoin.conf"), 'w', encoding='utf8') as f:
...
def append_config(datadir, options):
    with open(os.path.join(datadir, "fittexxcoin.conf"), 'a', encoding='utf8') as f:
...
def get_auth_cookie(datadir, chain):
    user = None
    password = None
    if os.path.isfile(os.path.join(datadir, "fittexxcoin.conf")):
        with open(os.path.join(datadir, "fittexxcoin.conf"), 'r', encoding='utf8') as f:
            for line in f:
