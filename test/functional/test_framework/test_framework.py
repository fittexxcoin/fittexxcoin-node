...
class FittexxcoinTestMetaClass(type):
    """Metaclass for FittexxcoinTestFramework.

    Ensures that any attempt to register a subclass of `FittexxcoinTestFramework`
    adheres to a standard whereby the subclass overrides `set_test_params` and
    `run_test` but DOES NOT override either `__init__` or `main`. If any of
    those standards are violated, a ``TypeError`` is raised."""

    def __new__(cls, clsname, bases, dct):
        if not clsname == 'FittexxcoinTestFramework':
            if not ('run_test' in dct and 'set_test_params' in dct):
                raise TypeError("FittexxcoinTestFramework subclasses must override "
                                "'run_test' and 'set_test_params'")
            if '__init__' in dct or 'main' in dct:
                raise TypeError("FittexxcoinTestFramework subclasses may not override "
                                "'__init__' or 'main'")

        return super().__new__(cls, clsname, bases, dct)


class FittexxcoinTestFramework(metaclass=FittexxcoinTestMetaClass):
    """Base class for a fittexxcoin test script.

    Individual fittexxcoin test scripts should subclass this class and override the set_test_params() and run_test() methods.
    ...
    def main(self):
        ...
        parser.add_argument("--nocleanup", dest="nocleanup", default=False, action="store_true",
                            help="Leave fittexxcoinds and test.* datadir on exit or error")
        parser.add_argument("--noshutdown", dest="noshutdown", default=False, action="store_true",
                            help="Don't stop fittexxcoinds after the test execution")
        ...
        parser.add_argument("--usecli", dest="usecli", default=False, action="store_true",
                            help="use fittexxcoin-cli instead of RPC for all commands")
        ...
        parser.add_argument("--extra-fittexxcoind-args", dest="extra_fittexxcoind_args", default="",
                            help="Start fittexxcoind with these additional arguments (comma separated)")
        ...
        self.config = config
        self.options.fittexxcoind = os.getenv(
            "FITTEXXCOIND", default=config["environment"]["BUILDDIR"] + '/src/fittexxcoind' + config["environment"]["EXEEXT"])
        if sys.platform == "darwin":
            self.options.fittexxcoinqt = os.getenv(
                "FITTEXXCOINQT", default=config["environment"]["BUILDDIR"] + '/src/qt/FittexxcoinNode-Qt.app/Contents/MacOS/FittexxcoinNode-Qt' + config["environment"]["EXEEXT"])
        else:
            self.options.fittexxcoinqt = os.getenv(
                "FITTEXXCOINQT", default=config["environment"]["BUILDDIR"] + '/src/qt/fittexxcoin-qt' + config["environment"]["EXEEXT"])

        self.options.fittexxcoincli = os.getenv(
            "FITTEXXCOINCLI", default=config["environment"]["BUILDDIR"] + '/src/fittexxcoin-cli' + config["environment"]["EXEEXT"])
        ...
        if not self.options.noshutdown:
            ...
            self.log.info(
                "Note: fittexxcoinds were not stopped and may still be running")
    ...
    def add_nodes(self, num_nodes, extra_args=None,
        ...
        if binary is None:
            ...
            for args in extra_args:
                binary.append(self.options.fittexxcoinqt if "-ui" in args else self.options.fittexxcoind)
        # drop -ui from args
        ...
        for i in range(num_nodes):
            self.nodes.append(TestNode(
                ...
                fittexxcoind=binary[i],
                fittexxcoin_cli=self.options.fittexxcoincli,
            ...
            if len(self.options.extra_fittexxcoind_args):
                self.nodes[i].extend_default_args(
                    self.options.extra_fittexxcoind_args.split(","))

    def start_node(self, i, *args, **kwargs):
        """Start a fittexxcoind"""
    ...
    def stop_node(self, i, expected_stderr='', wait=0):
        """Stop a fittexxcoind test node"""
        self.nodes[i].stop_node(expected_stderr, wait=wait)
        self.nodes[i].wait_until_stopped()

    def stop_nodes(self, wait=0):
        """Stop multiple fittexxcoind test nodes"""
        for node in self.nodes:
    ...
    def _start_logging(self):
        ...
        ch.setLevel(ll)
        # Format logs the same as fittexxcoind's debug.log with microprecision (so
        ...
        if self.options.trace_rpc:
            rpc_logger = logging.getLogger("FittexxcoinRPC")
    ...
    def _initialize_chain(self):
        ...
        if create_cache:
            ...
            # Create cache directories, run fittexxcoinds:
            for i in range(MAX_NODES):
                ...
                self.nodes.append(TestNode(
                    ...
                    p2p_port=p2p_port(i),
                    timewait=self.rpc_timeout,
                    fittexxcoind=self.options.fittexxcoind,
                    fittexxcoin_cli=self.options.fittexxcoincli,
                    mocktime=self.mocktime,
                    coverage_dir=None,
                    emulator=self.options.emulator,
        ...
        for i in range(self.num_nodes):
            ...
            # Overwrite port/rpcport in fittexxcoin.conf
    ...
    def skip_if_no_fittexxcoind_zmq(self):
        """Skip the running test if fittexxcoind has not been compiled with zmq support."""
        if not self.is_zmq_compiled():
            raise SkipTest("fittexxcoind has not been built with zmq enabled.")
    ...
    def skip_if_no_cli(self):
        """Skip the running test if fittexxcoin-cli has not been compiled."""
        if not self.is_cli_compiled():
            raise SkipTest("fittexxcoin-cli has not been compiled.")

    def is_cli_compiled(self):
        """Checks whether fittexxcoin-cli was compiled."""
        config = configparser.ConfigParser()
        config.read_file(open(self.options.configfile, encoding='utf-8'))
